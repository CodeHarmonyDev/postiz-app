...
type LegacyPreviewResponse = Array<{
  id: string;
  content: string;
  publishDate: string;
  image?: string;
...
function normalizeLegacyPreview(data: LegacyPreviewResponse): NormalizedPreview | null {
  if (!Array.isArray(data) || !data.length || !data[0]?.integration) {
    return null;
  }

  return {
    source: 'legacy',
    integration: data[0].integration,
    posts: data.map((post) => ({
      id: String(post.id),
...
export function PublicPostPreview({
  ...
  const t = useT();
  const convexPreview = useQuery(api.posts.getPublicPreview, {
    postId,
  });
  const [legacyPreview, setLegacyPreview] = useState<NormalizedPreview | null>();
  const [isLoadingLegacy, setIsLoadingLegacy] = useState(false);

  useEffect(() => {
    if (convexPreview !== null || legacyPreview !== undefined || isLoadingLegacy) {
      return;
    }

    setIsLoadingLegacy(true);
    void fetch(`/public/posts/${postId}`)
      .then(async (response) => {
        if (!response.ok) {
          return null;
        }

        return normalizeLegacyPreview((await response.json()) as LegacyPreviewResponse);
      })
      .then((preview) => {
  ...
  }, [convexPreview, isLoadingLegacy, legacyPreview, postId]);

  const preview = useMemo<NormalizedPreview | null>(() => {
    if (convexPreview) {
      return {
        source: 'convex',
        integration: convexPreview.integration,
        posts: convexPreview.posts,
      };
    }

    return legacyPreview || null;
  }, [convexPreview, legacyPreview]);

  if (convexPreview === undefined || isLoadingLegacy) {
    return null;
  }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-sm text-gray-400 flex items-center gap-[20px]">
            ...
            <div className="flex-1">
              ...
              <RenderPreviewDateClient date={preview.posts[0].publishDate} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row text-white w-full max-w-[1346px] mx-auto">
                    ...
                    <div className="flex shrink-0 rounded-full h-30 w-30 relative">
                      <div className="w-[50px] h-[50px] z-[20]">
                        <img
                          ...
                          alt={preview.integration.name}
                          src={preview.integration.picture}
                        />
                      </div>
                      <div className="absolute -end-[5px] -bottom-[5px] w-[30px] h-[30px] z-[20]">
                        <img
                          ...
                          alt={preview.integration.providerIdentifier}
                          src={`/icons/platforms/${preview.integration.providerIdentifier}.png`}
                        />
                      </div>
                    </div>
                  </div>
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center space-x-2">
                      <h2 className="text-sm font-semibold">
                        {preview.integration.name}
                      </h2>
                      <span className="text-sm text-gray-500">
                        @{preview.integration.profile}
                      </span>
                    </div>
        ...
        <div className="w-full lg:w-96 lg:flex-shrink-0">
          <div className="p-4 pt-0">
            <CommentsComponents postId={postId} source={preview.source} />
          </div>
        </div>
      </div>
    </div>
